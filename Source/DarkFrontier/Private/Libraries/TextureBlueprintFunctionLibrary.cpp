...
#include "Libraries/TextureBlueprintFunctionLibrary.h"
#include "Libraries/AssetBlueprintFunctionLibrary.h"

void UTextureBlueprintFunctionLibrary::SaveMipTexture(UTexture2D* Source, const int32 MipFactorX, const int32 MipFactorY)
{
	UPackage* SourcePackage = Source->GetPackage();
	const FString SourceFullName = UAssetBlueprintFunctionLibrary::GetPackageFullName(SourcePackage);
	const FString SourceName = UAssetBlueprintFunctionLibrary::GetPackageName(SourcePackage);
	...
	UPackage* DestPackage = UAssetBlueprintFunctionLibrary::GetPackage(DestFullName);
	...
	UAssetBlueprintFunctionLibrary::NewAsset(DestPackage, Dest);
}

// Use int32 instead of uint32 as parameter type as only int32 is supported by blueprints
void UTextureBlueprintFunctionLibrary::InitMipTexture(UTexture2D* Dest, UTexture2D* Source, const int32 MipFactorX, const int32 MipFactorY)
{
	// Get texture sizes
